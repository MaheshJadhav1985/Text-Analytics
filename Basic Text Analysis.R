...
stpw2 # tm package stop word list; tokenizer package has the same name function, hence 'tm::'
comn  = unique(c(stpw1, stpw2))         # Union of two list
stopwords = unique(gsub("'"," ",comn))  # final stop word lsit after removing punctuation
class(stopwords)

x  = text.clean(data$text)
# applying func defined above to pre-process text corpus
x  = removeWords(x,stopwords)# removing stopwords created above
x  =  stripWhitespace(x)                  # removing white space
# y  =  stemDocument(x)                   # can stem doc if needed.
head(x,15)
# #--------------------------------------------------------#
...
# Below code tokenizes the dicuments in corpus
it_0 = itoken( x,
                  ...
                  progressbar = T)
it_0
# Below code tokenizes the documents in corpus

vocab = create_vocabulary(it_0,    #  func collects unique terms & corresponding statistics
                          ngram = c(2L, 2L) #, this is to form bigrams(2l,2l) implies min 2 and max 2( that unigrams, trigrams and above are not considered)
                          #stopwords = stopwords
                          )
class(vocab)
length(vocab); str(vocab);
class(vocab)# view what vocab obj is like
#below code filters out only those bigrams which occur 10 or more times
...
text2 = x
text2 = paste("",text2,"")#paste converts its arguments (via as.character) to character strings, and concatenates them (separating them by the string given by sep). If the arguments are vectors, they are concatenated term-by-term to give a character vector result. Vector arguments are recycled as needed, with zero-length arguments being recycled to "".
...
dtm = dtm[,order(apply(dtm, 2, sum), decreasing = T)] 
dtm
# sorting dtm's columns in decreasing order of column sums
...
distill.cog = function(mat1, # input TCM ADJ MAT
  ...
  plot(graph, 
       layout = layout.kamada.kawai, #algo for drawing gen undirected graphs
       main = title)
